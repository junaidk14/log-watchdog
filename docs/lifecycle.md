...
5. An old Demo incident/log/delivery URL carrying the previous run explains that the run was reset and links to the current Demo. Numeric identities are not reused, so older untagged incident links cannot attach to a new incident either.
