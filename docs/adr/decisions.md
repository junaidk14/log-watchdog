...
## ADR-032 — Session-only Gemini key entry

- **Decision:** Accept a Gemini key through the local UI into backend memory only, with configured/not-configured status and Clear key. Prefer that override over the startup environment key; clearing restores the environment fallback. Keep provider model/paid settings in the environment and preserve preview → explicit send. Reject key changes during sends and invalidate prior previews on changes.
- **Rationale:** The user explicitly requested trying optional analysis without shell configuration or restart while prohibiting credential persistence or disclosure. This changes the earlier environment-only credential restriction, not the application architecture or provider scope.
- **Status:** Accepted and implemented in the bounded cleanup; supersedes the environment-only key-source portion of ADR-020/021. No real key or live provider call was used for verification.
