...
- Ordinary test, build, verification and review failures belong to the implementation/fix workflow: diagnose, fix and rerun them within the configured retries. They do not alone require BLOCKED. Stop only when failures remain unresolved after configured retries, there is a genuine blocker, or Git/PR state is unsafe or ambiguous. Preserve logs and checkout; do not weaken checks or increase retry limits.
- Expected open prerequisites in the graph below are normal sequencing constraints. If an otherwise eligible assigned issue is genuinely blocked, verification remains unresolved after configured retries, dependencies cannot be verified, or repository state is unsafe or ambiguous, report BLOCKED and stop with logs and work preserved. Do not skip a newly blocked issue to continue unrelated work or weaken any gate. If assigned issues remain open but none is executable, report BLOCKED rather than declaring completion.
...
- Bootstrap any test/build commands owned by the current issue as usual. The absence of an existing frontend scaffold or test script is not permission to skip its creation. Unresolved failures or missing required non-browser checks still block PR READY; diagnose and fix ordinary failures before handoff; all other verification requirements remain unchanged.
